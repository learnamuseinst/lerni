...
interface EditBujoModalProps {
  entry: BujoEntry | null;
  isOpen: boolean;
  onClose: () => void;
  onSave: (updatedEntry: BujoEntry) => void;
  categories?: string[];
  onOpenSettings?: () => void;
}
...
export const EditBujoModal: React.FC<EditBujoModalProps> = ({
  entry,
  isOpen,
  onClose,
  onSave,
  ...
  useEffect(() => {
    if (entry && isOpen) {
      ...
      setType(entry.type);
      setTargetDate(entry.date || todayStr);
      setTargetTime(entry.time || '');
      setDurationMinutes(entry.durationMinutes ? String(entry.durationMinutes) : '');
      setEnergyCost(entry.energyCost || 'low');
      setIsPriority(Boolean(entry.isPriority));
      setCategory(entry.category || availableCategories[0] || 'General');
      setTimeout(() => {
        inputRef.current?.focus();
      }, 100);
    }
  }, [entry, isOpen, todayStr, availableCategories]);
      ...
      >
        ...
        <div className="flex items-center justify-end gap-2 pt-3 border-t border-[#DCEAD4] dark:border-[#263722]">
          ...
          >
            ...
            <span>{t('journal.saveEntryBtn')}</span>
          </motion.button>
        </div>
